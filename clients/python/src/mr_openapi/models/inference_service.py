...
class InferenceService(BaseModel):
    ...
    model_version_id: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="ID of the `ModelVersion` to serve. If it's unspecified, then the latest `ModelVersion` by creation order will be served.", alias="modelVersionId")
    runtime: Optional[StrictStr] = Field(default=None, description="Model runtime.")
    desired_state: Optional[InferenceServiceState] = Field(default=InferenceServiceState.DEPLOYED, alias="desiredState")
    registered_model_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="ID of the `RegisteredModel` to serve.", alias="registeredModelId")
    serving_environment_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="ID of the parent `ServingEnvironment` for this `InferenceService` entity.", alias="servingEnvironmentId")
    __properties: ClassVar[List[str]] = ["customProperties", "description", "externalId", "name", "id", "createTimeSinceEpoch", "lastUpdateTimeSinceEpoch", "modelVersionId", "runtime", "desiredState", "registeredModelId", "servingEnvironmentId"]
    ...
    def model_version_id_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[1-9][0-9]{0,8}$", value):
            raise ValueError(r"must validate the regular expression /^[1-9][0-9]{0,8}$/")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "desiredState": obj.get("desiredState") if obj.get("desiredState") is not None else InferenceServiceState.DEPLOYED,
